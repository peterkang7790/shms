...
<%--				<tr>--%>
<%--					<td height="17" background="../images/panel_menu_bg.gif"><span class="quick">--%>
<%--						<a href="javascript:moveLeftMenu(5000,'<%=basePath %>statManage/statDaily.do');"  onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Image6','','../images/menu_04o.gif',1)" onfocus="this.blur();">통계</a></span></td>--%>
<%--				</tr>--%>
<%--				<tr>--%>
<%--					<td><img src="../images/panel_down.gif" width="95" height="12"></td>--%>
<%--				</tr>--%>
...
//    window.setInterval(quickOth, 10000);
...
{ 
	$.ajax({ 
		...
		success: function(html){
			...
			if( html == 1 )
			{
				jQuery("#changeGif").attr('src','../images/panel_check_red.gif');
...
//				beepM(2,html);
		...
		//		jQuery("#hems_alm").attr('src','');
			...
			else if( html == 2 )
			{
				...
				jQuery("#changeGif").attr('src','../images/panel_check_yellow.gif');
	...
	$.getJSON("http://hemsfos.lsware.co.kr/shms/otherStructPop.do",
		       function(data){
					for(i=0; i<data.list.length; i++)
					{
						if( data.list[i].tr == "1" )
						{
							window.showModalDialog('../common/otherStruct.do?trclass='+data.list[i].tr+'&struct_id='+data.list[i].id+'&struct_nm='+data.list[i].nm,'','dialogHeight:100px; dialogWidth:100px; status:no;scroll:no; help:no;edge:sunken');
//							MM_openBrWindow( '../common/otherStruct.do?trclass='+data.list[i].tr+'&struct_id='+data.list[i].id+'&struct_nm='+data.list[i].nm,'','width=200,height=200, resizable=yes, scrollbars=no');
							jQuery("#hems_alm").attr('src','../sound/wav33.wav');
						}
